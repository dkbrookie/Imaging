...
$windowslogs = "$env:windir\LTSvc\packages\OS\Win10-$win10Build-Logs"
$Dir = "$env:windir\LTSvc\packages\OS\Win10\$win10Build"
$ISO = "$Dir\Pro$osArch.$win10Build.iso"
$isoMountURL = "https://drive.google.com/uc?export=download&id=1XpZOQwH6BRwi8FpFZOr4rHlJMDi2HkLb"
$isoMountExe = "$Dir\mountIso.exe"
$SetupComplete = "$Dir\SetupComplete.cmd"
...
$checkISO = Test-Path $ISO -PathType Leaf
If ($checkISO) {
...
Try {
    ...
    If ($status -eq 'Install') {
        ...
        If ($osVer -like '*10*' -or $osVer -like '*8.1*') {
            ...
            ## Mount the ISO
            cmd.exe /c "echo . | $isoMountExe $ISO" | Out-Null
        }
